...
walk_randomly = function(self, dtime)
	if self.time_counters[1] >= FIND_PATH_TIME_INTERVAL then
		...
		or self:has_item_in_main(function(itemname)	return (minetest.get_item_group(itemname, "seed") > 0) end) then
			local destination = working_villages.func.search_surrounding(self.object:getpos(), is_plantable_place, searching_range)
		...
		local destination = working_villages.func.search_surrounding(self.object:getpos(), is_mowable_place, searching_range)
		if destination ~= nil then
			local path = minetest.find_path(self.object:getpos(), destination, 10, 1, 1, "A*")
...
walk_to_plant_and_mow_common = function(self, dtime)
	...
	if is_near(self, self.path[1], 0.5) then
		...
		-- if villager is stopped by obstacles, the maidroid must jump.
		local velocity = self.object:getvelocity()
		if velocity.y == 0 then
			local front_node = self:get_front_node()
...
working_villages.register_job("working_villages:job_farmer", {
	description      = "working_villages job : farmer",
