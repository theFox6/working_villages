...
function hider.can_traverse(v, pos)
  ...
  local path = pathfinder.find_path(position,pos,v)
...
function hider.can_fit(pos)
  ...
  if not func.walkable_pos(pos) then return false end

  up  = vector.add(pos, up_vec1)
  if not func.walkable_pos(up) then return false end
...
working_villages.register_job("working_villages:job_hider1", {
  description      = "hider v1 (working_villages)",
